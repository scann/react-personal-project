...
describe('Компонент Task:', () => {
    describe('должен иметь базовую разметку JSX:', () => {
        ...
        describe('оберточный элемент <li />:', () => {
            test('должен быть один', () => {
                expect(result.find('li').length).toBe(1);
            });

            test('должен иметь CSS-класс task', () => {
                expect(result.find('li').hasClass('task')).toBe(true);
            });

            test('если иметь дополнительный CSS-класс completed, если значение пропса completed — true', () => {
                result.setProps({
                    completed: true,
                });

                expect(result.find('li').hasClass('completed')).toBe(true);
                result.setProps({
                    completed: false,
                });
            });
        ...
        describe('layout-элемент div с CSS-классом content', () => {
            test('должен быть один', () => {
                expect(result.find('div.content').length).toBe(1);
            });

            test('должен иметь один компонент <Checkbox /> в качестве потомка', () => {
                expect(result.find('div.content').find('Checkbox').length).toBe(
                    1,
                );
            });

            test('компоненту <Checkbox /> нужно передать пропс className = { Styles.toggleTaskCompletedState }', () => {
                expect(result.find('Checkbox').prop('className')).toBe(
                    'toggleTaskCompletedState',
                );
            });

            test('должен иметь один элемент <input /> в качестве потомка', () => {
                expect(result.find('div.content > input').length).toBe(1);
            });

            test('элемент <input /> должен быть текстового типа', () => {
                expect(result.find('div.content > input').prop('type')).toBe(
                    'text',
                );
            });

            test('максимальная длина текстового контента элемента <input /> — 50 символов (необходимо использовать атрибут maxLength)', () => {
                expect(
                    result.find('div.content > input').prop('maxLength'),
                ).toBe(50);
            });

            test('элемент <input /> должен быть не редактируемым по-умолчанию, необходимо использовать атрибут disabled', () => {
                expect(
                    result.find('div.content > input').prop('disabled'),
                ).toBe(true);
            });

            test('элемент <input /> должен прослушивать событие onChange обработчиком this._updateNewTaskMessage', () => {
                expect(
                    result.find('div.content > input').prop('onChange'),
                ).toBe(result.instance()._updateNewTaskMessage);
            });

            test('элемент <input /> должен прослушивать событие oonKeyDown обработчиком this._updateTaskMessageOnKeyDown', () => {
                expect(
                    result.find('div.content > input').prop('onKeyDown'),
                ).toBe(result.instance()._updateTaskMessageOnKeyDown);
            });
        });

        describe('layout-элемент div с CSS-классом actions', () => {
            test('должен быть один', () => {
                expect(result.find('div.actions').length).toBe(1);
            });

            test('должен иметь один компонент <Star /> в качестве потомка', () => {
                expect(result.find('div.actions').find('Star').length).toBe(1);
            });

            test('компоненту <Star /> нужно передать пропс className = { Styles.toggleTaskFavoriteState }', () => {
                expect(result.find('Star').prop('className')).toBe(
                    'toggleTaskFavoriteState',
                );
            });

            test('должен иметь один компонент <Edit /> в качестве потомка', () => {
                expect(result.find('div.actions').find('Edit').length).toBe(1);
            });

            test('компоненту <Edit /> нужно передать пропс className = { Styles.updateTaskMessageOnClick }', () => {
                expect(result.find('Edit').prop('className')).toBe(
                    'updateTaskMessageOnClick',
                );
            });

            test('должен иметь один компонент <Remove /> в качестве потомка', () => {
                expect(result.find('div.actions').find('Remove').length).toBe(
                    1,
                );
            });

            test('компоненту <Remove /> нужно передать пропс className = { Styles.removeTask }', () => {
                expect(result.find('Remove').prop('className')).toBe(
                    'removeTask',
                );
            });
        });
    ...
    describe('должен содержать базовые методы класса, реализированные через методы стрелки (свойства класса)', () => {
        ...
        describe('_updateTaskMessageOnKeyDown', () => {
            ...
            test('не должен совершать никаких операций и возвращать null, если свойство состояние newMessge — пустая строка', () => {
                result.setState({
                    newMessage: '',
                });
